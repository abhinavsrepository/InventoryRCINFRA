...
class employeeClass:
    def __init__(self,root):
        ...
        cmb_gender=ttk.Combobox(self.root,textvariable=self.var_searchby,values=("Select","Male","Female","Other"),state='readonly',justify=CENTER,font=("goudy old style",15))
  ...
  #===================================================================================row4=========================================================================================================================================
        ...
        self.EmployeeTable=ttk.Treeview(emp_frame,columns=("eid","name","gender","email","contact","dob","doj","pass","utype","address","salary"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                row =cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This EmployeeID already assigned, try different id!",parent=self.root)
                else:
                    cur.execute("Insert into employee(eid,name,gender,email,contact,dob,doj,pass,utype,address,salary)values(?,?,?,?,?,?,?,?,?,?,?)",(
                                        self.var_emp_id.get(),
                                        self.var_name.get(),
                                        self.var_email.get(),
                                        self.var_gender.get(),
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                if row==None:
                    ...
                    cur.execute("Update employee set name=?,gender=?,email=?,contact=?,dob=?,doj=?,pass=?,utype=?,address=?,salary=? where eid=?",(
            ...
            if self.var_emp_id.get()=="":
                ...
                if row==None:
                    ...
                    if op==True:
                        ...
                        con.commit()
                        messagebox.show.showinfo("Delete", "Employee Deleted Successfully",parent=self.root)
